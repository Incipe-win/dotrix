...
int SetupCommand::run_list(const std::vector<Recipe>& all) {
    ...
    for (auto& t : all) {
        ...
        bool has = !util::capture({"which", check_cmd}).empty()
                || util::run({"sh", "-c", check_cmd + " >/dev/null 2>&1"});
